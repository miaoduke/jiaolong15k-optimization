...
#define UNIWILL_FEATURE_KEYBOARD_BACKLIGHT	BIT(9)
...
struct uniwill_data {
	...
	struct mutex kbd_rgb_led_lock;	/* Protects writes to the keyboard RGB registers */
...
/* ===================== Keyboard backlight (ported from linux-next, GM5BG0E) ===================== */
...
static int uniwill_kbd_led_read_brightness(struct uniwill_data *data)
{
	...
	return min(FIELD_GET(KBD_BRIGHTNESS_MASK, regval), 4);
}
...
/*
 * GM5BG0E: brightness is deliberately NOT written to the EC here. The EC
 * firmware handles Fn+F6/F7 brightness natively (hardware mode) and any
 * write with KBD_APPLY switches it into software mode where Fn+F6/F7 stop
 * working (no WMI OSD events on this machine). The brightness LED field is
 * therefore read-only (mirrors the hardware value set by Fn+F6/F7).
 */
...
static int uniwill_kbd_led_init(struct uniwill_data *data)
{
	...
	int ret;
		...
		if (!(supported_features & UNIWILL_FEATURE_KEYBOARD_BACKLIGHT))
		return 0;
	...
	data->kbd_led_mc_cdev.led_cdev.max_brightness = 4;
