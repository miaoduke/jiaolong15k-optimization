...
> **🆙 Upstream keyboard-IRQ patch submitted / 🆙 键盘 IRQ 修复已提交上游:** The PS2-keyboard IRQ-polarity fix for this board (`GM5BG0E`) was submitted to the Linux kernel as an ACPI quirk on **2026-08-27**: `[PATCH v2] ACPI: resource: Add MECHREVO GM5BG0E to irq1_edge_low_force_override[]`, sent to `linux-acpi@vger.kernel.org` + `Cc: stable`. After Greg KH asked *"Real name please."*, the author signed as **DUAN Xuejian（段雪健）**. v2 thread: Message-ID `<tencent_262C335D5C0549C6ED9EA88FF00437B26708@qq.com>`, retrievable at `https://lore.kernel.org/linux-acpi/?q=MECHREVO+GM5BG0E`. **Merge status (2026-09-01): pending maintainer review** — the table in mainline `drivers/acpi/resource.c` still lacks `GM5BG0E`; the v2 thread shows no follow-ups yet. Full trace: `03_代码_Linux/固件修复_键盘/README.md`. Until upstream merges, this repo keeps the **DSDT Override** (方案1) as the deployed fix. / 本主板（`GM5BG0E`）的 PS2 键盘 IRQ 极性修复已于 **2026-08-27** 作为 ACPI quirk 提交到 Linux 内核：`[PATCH v2] ACPI: resource: Add MECHREVO GM5BG0E to irq1_edge_low_force_override[]`，发往 `linux-acpi@vger.kernel.org` 并抄送 `stable`。在 Greg KH 回信要求 *"Real name please."* 后，作者改以真名 **DUAN Xuejian（段雪健）** 署名提交。v2 线程：Message-ID `<tencent_262C335D5C0549C6ED9EA88FF00437B26708@qq.com>`，可在 `https://lore.kernel.org/linux-acpi/?q=MECHREVO+GM5BG0E` 检索。**合入状态（2026-09-01）：待维护者评审**——主线 `drivers/acpi/resource.c` 的表仍缺 `GM5BG0E`，v2 线程尚无跟进回复。完整追溯：`03_代码_Linux/固件修复_键盘/README.md`。上游合入前，本仓库沿用 **DSDT Override（方案1）** 作为已部署的持久修复。
