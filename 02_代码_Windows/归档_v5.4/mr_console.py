...
BROKER_PORT = 13688
CLIENT_SLOT = 5                      # PluginClient_5, 避开官方占用的低槽位
...
class MiniMQTT:
    ...
    def _connect_once(self, timeout=5) -> bool:
        ...
        if not ack or len(ack) < 4 or ack[0] >> 4 != 2 or ack[3] != 0:
            ...
            raise ConnectionError(f"CONNACK 被拒 returnCode={rc}")
        self._run = True
        self._rx.start()
...
class MrConsole:
    ...
    def set_field(self, ctrl_topic, field, value, extra=None):
        """写入口: Fan/Control 的字段自动翻译为真实指令 SET_OPERATING_MODE_DETAIL(抓包实证)"""
...
def cli():
    ...
    if cmd == "gui":
        import mr_gui_v5                      # v5.3修复: 原误写 mr_gui(不存在)
        mr_gui_v5.run(app_factory=lambda: app)
        return
